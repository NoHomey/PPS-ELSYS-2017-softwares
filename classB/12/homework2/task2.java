...
public class MatrixMultiply {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        if(!checkIfMultipliable(matrix1, matrix2)) {
            System.out.print("undefined");
        } else {
            Vector<Vector<Integer>> result = multiplyTwoMatrices(matrix1, matrix2);

            if(checkIfAsymmetric(result)) {
    ...
    private static boolean checkIfAsymmetric(Vector<Vector<Integer>> matrix) {
    ...
    private static Vector<Vector<Integer>> multiplyTwoMatrices(Vector<Vector<Integer>> matrix1, Vector<Vector<Integer>> matrix2) {
        ...
        for(int i = 0; i < matrix1.size(); i++) {
            ...
            for(int j = 0; j < matrix2.get(0).size(); j++) {
                ...
                for(int k = 0; k < matrix1.get(0).size(); k++) {
                    val += matrix1.get(i).get(k) * matrix2.get(k).get(j);
                }
                row.add(val);
            }
            result.add(row);
    ...
    private static boolean checkIfMultipliable(Vector<Vector<Integer>> matrix1, Vector<Vector<Integer>> matrix2) {
